...
3. `npm run qa:apps` prüft aktuell keine App-Deployment-URLs, weil `ADMIN_BASE_URL`, `GUEST_BASE_URL` und `OWNER_BASE_URL` fehlen.
...
- `npm run qa:apps` prüft Admin-, Guest- und Owner-Production- oder Staging-URLs tatsächlich mit `checkedApps > 0`.
...
3. Automatische Gates aus `docs/ADMIN_PARITY_QA_RUNBOOK.md` ausführen.
4. Die 24 manuellen Gates mit Evidenz abnehmen.
5. Ergebnis grün/gelb/rot dokumentieren.
