create table if not exists public.owner_statements (
  ...
  period_start date,
  period_end date,
  status text not null default 'draft' check (status in ('draft', 'visible', 'paid', 'archived')),
  currency text not null default 'EUR',
  gross_revenue numeric(12, 2) not null default 0,
  morrow_fee numeric(12, 2) not null default 0,
...
create index if not exists owner_statements_period_idx
  on public.owner_statements (period_start desc, created_at desc);

alter table public.owner_statements enable row level security;

grant select, insert, update, delete on table public.owner_statements to authenticated, service_role;

drop policy if exists "owners can read visible assigned owner statements" on public.owner_statements;
create policy "owners can read visible assigned owner statements"
  on public.owner_statements for select
  to authenticated
  using (
    status in ('visible', 'paid')
    and exists (
      select 1
      ...
      where owner_profile.status = 'active'
        and owner_access.property_id = owner_statements.property_id
        and owner_access.can_view_financials = true
        and (
...
drop policy if exists "admin users can manage owner statements" on public.owner_statements;
create policy "admin users can manage owner statements"
  on public.owner_statements for all
  to authenticated
  using (public.is_morrow_admin())
  with check (public.is_morrow_admin());

drop policy if exists "service role can manage owner statements" on public.owner_statements;
create policy "service role can manage owner statements"
  on public.owner_statements for all
  to service_role
  using (true)
  with check (true);
