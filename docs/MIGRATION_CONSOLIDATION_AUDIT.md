...
| Shared Domain/Types | verstreut in `src/App.tsx`, Apps | `packages/domain`, `packages/supabase`, `docs/PAYLOAD_NORMALIZATION_INVENTORY.md` | teilweise | kritisch | Gemeinsame Supabase-Typanker fuer JSON-Payloads, `local_places`, Kommunikation, Support, Erlebnisbausteine, Anbieter, Agenturen sowie Owner-Dokumente, Abrechnungen und Operations werden von Admin/Guest/Owner schrittweise genutzt. Audit-Insert ist zentralisiert und Payload-Normalisierung ist inventarisiert. Einige Admin-/Guest-/Owner-Zeilen, Mapper und Payload-Konventionen liegen aber weiter app-lokal; Risiko fuer Logikdrift bleibt reduziert, aber nicht erledigt. |
