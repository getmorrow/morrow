...
| Unterkunftsverwaltung | Vite Owner/Property/Profile | `apps/admin` Bestand/Objekte | weitgehend migriert | kritisch | Objektprofil, Check-in, Regeln, Ausstattung, Attribute, Erlebniswelten, Medienrechte, Objektpruefung und verknuepfte Auszeiten sind vorhanden; Medienbibliothek, Upload, Bildreihenfolge und spaetere Normalisierung bleiben offen. |
| Erlebnisbausteine | Vite `experiences` | `apps/admin` Erlebnisse | weitgehend migriert | hoch | Anbieterzuordnung, Rolle, Inklusivstatus, Preis-/Kapazitaets-/Verfuegbarkeitsnotiz, Gastnotiz, Readiness-Pruefung und Qualitaetsnotiz/-score vorhanden; Qualitaetshistorie, echte Verfuegbarkeitsstruktur und spaetere Normalisierung offen. |
...
| Vor-Ort-Orte | Vite `localPlaces`, `src/data/localPlaces.ts` | `apps/admin` Orte + `apps/guest` Vor Ort | weitgehend migriert | hoch | Kuratierung, Freigabe, Karte, Links und Gastansicht vorhanden; Admin prueft fehlende Koordinaten, Beschreibung, Bilder und kategoriespezifische Pflichtfelder. Live-Oeffnungszeiten/Ratings und Medienqualitaet bleiben offen. |
...
- Spam-Policy, Marketing-Consent und Reisegruppen-Normalisierung pruefen; Wiedervorlage und WhatsApp-Kontaktzustimmung sind normalisiert.
...
6. Lead-Spam-/Loeschpolicy, Marketing-Consent und Reisegruppen-Normalisierung pruefen/entscheiden; `leads.follow_up_at` und WhatsApp-Kontaktzustimmung sind bereits normalisiert.
