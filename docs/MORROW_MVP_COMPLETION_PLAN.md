...
- `apps/web` proxyt `/admin`, `/app/gast` und `/app/eigentuemer` per `MORROW_ADMIN_APP_URL`, `MORROW_GUEST_APP_URL` und `MORROW_OWNER_APP_URL` auf die jeweiligen App-Projekte. Nach außen bleibt `www.getmorrow.de` sichtbar.
- Legacy-Pfade `/deine-auszeit/...`, `/owner`, `/app/guest` und `/app/owner` leiten auf die deutschen Plattformpfade weiter.
- Alle vier Next-Apps haben einen Health-Endpunkt zur Deployment- und App-Identitaetspruefung; öffentlich liegen sie unter `/health`, `/admin/health`, `/app/gast/health` und `/app/eigentuemer/health`.
