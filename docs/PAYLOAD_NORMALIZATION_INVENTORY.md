...
| --- | --- | --- | --- | --- | --- |
| `leads` | `selectedDate`, `adults`, `children`, `childrenAges`, `dog`, `occasion`, `internalNote`, `utm`, `updatedAt`, Testmarker; `followUpAt` und `whatsappOptIn` nur noch als Fallback/Spiegel | Admin, Web-Leadflow | Admin | `follow_up_at`, `whatsapp_opt_in` und `whatsapp_consent_at` normalisiert; `Normalisieren V1` fuer Reisegruppe; `MVP-Payload ok` fuer Anlass/Notizen/UTM | Wiedervorlage und WhatsApp-Zustimmung liegen strukturiert vor und werden aus altem Payload backfilled. Reisegruppe bleibt naechster Lead-V1-Kandidat. UTM kann Payload bleiben, solange Reporting nicht granular wird. |
